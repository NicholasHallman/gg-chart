...
const COLORS = Array(11).fill(0).map((_, i) => `var(--gg-color-${i + 1})`);

export const runProjection = (host, chart) => {
  const {aes, scale, data, geom, width, height, coord, pos, stats, legend} = host;
  const {innerHeight, innerWidth} = chartSizes(width, height, scale, coord, aes, legend);
  ...
  const points = data.map( r => {
    const point = {
      ...
      click: () => { chart.dispatchEvent(new CustomEvent('gg-chart-point-click', {detail: {point}})) }
    };
    aes.forEach( a => {point[a] = r[a]});


    if(coord.flip) {
      // Flip projection
      point.px = ((point[aes[0]] - xlim[0]) * innerWidth / (xlim[1] - xlim[0]));
      point.py = innerHeight - (((point[aes[1]] - ylim[0]) * innerHeight / (ylim[1] - ylim[0])));
      ...
      point.width = geom.includes('bar') ? (innerHeight / (limDiff / breakSize) ) - 2 : 0;
    } else if(coord.polar) {
      // Polar projections
      ...
      point.width = geom.includes('bar') ? (innerWidth / (limDiff / breakSize) ) - 2 : 0;
      point.height = (innerHeight) - point.py;
    }

    if(aes.specialEntries().length !== 0) {
      if(aes.color){
        const colorValues = Array.from(new Set(data.map(re => re[aes.color]))).sort((a,b) => a - b);
      ...
      if(aes.color && geom.includes('bar')) {
        if(pos.dodge) {
        ...
        } else if((pos.stacked || (!pos.stacked && !pos.dodge)) && !coord.polar ) {
          // stacked bar chart
          ...
          point.width *= stats[aes[1]]?.parts ?? Math.min(innerWidth / 2, innerHeight / 2);
        }
      }
    }
