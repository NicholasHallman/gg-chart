...
class GGChart extends LitElement {
  ...
  render() {
    ...
    const sizing = chartSizes(this.width, this.height, this.transformedGrammar.scale, this.transformedGrammar.coord, this.transformedGrammar.aes, this.legend);
    ...
    const chartWidth = this.legendCanvas.length > 0 && !this.legend.includes('hidden') ? this.width - (this.width / 4) : this.width;
    return html`
      ${ this.title && html`<h3 class="d2l-heading-3 d2l-typography">${this.title}</h3>` }
      <div class="sbs" style="width: ${this.width}px" @mouseleave="${this.handleMouseLeave}">
        ...
        <div class="legend" style="width: ${this.width / 4}px; display: ${this.legendCanvas.length > 0 && !this.legend.includes('hidden') ? 'block' : 'none'}">${this.legendCanvas}</div>
        <gg-chart-tooltip
          style="top: ${sizing.padding / 2}px; left: ${sizing.yAxisSpacing + (sizing.padding / 2)}px"
