...
export class Pipeline {
  ...
  runPipeline(grammar) {
    return this.functions.reduce((g, func) => func({...g}, this.host), grammar)
  }

  initialize() {
    const {aes, scale, geom, pos, coord, stat, data, width, height, title, tooltip, sizing, legend} = this.host;
    const grammar = {
      aes,
      scale: scale ? [...scale] : [],
      geom,
